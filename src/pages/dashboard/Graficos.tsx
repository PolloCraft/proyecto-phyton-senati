...
function Graficos() {
  ...
  const generarGrafico = async (tipoGrafico: TipoGrafico, multi = false) => {
    ...
    try {
      ...
      let filteredRaw = guardados.csvRaw;

      py.globals.set("csv_content", filteredRaw);
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 8 }}>
                          {Object.entries(chartStats).map(([key, val]) => (
                            <div key={key} style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                              <span style={{ fontSize: 10, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.03em", fontWeight: 600 }}>
                                {statLabelMap[key] || key}
                              </span>
        ...
        {cargando ? (
          ...
          <div style={{ display: "flex", gap: 4, marginBottom: 16, background: "#f1f5f9", padding: 4, borderRadius: 8 }}>
            ...
            <button
              ...
              disabled={generando || cargandoPyodide}
              style={{
                flex: 1,
                padding: "8px 16px",
                borderRadius: 6,
                ...
                fontWeight: 600,
                fontSize: 13,
                cursor: generando || cargandoPyodide ? "wait" : "pointer",
                transition: "all 0.2s",
                fontFamily: "inherit",
                opacity: generando || cargandoPyodide ? 0.5 : 1,
              }}
            >
        ...
        >
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <ChartIcon type={expandedChart.type as TipoGrafico} size={22} style={{ color: "#1e3a5f" }} />
              <div>
