...
function Pandas() {
  ...
  useEffect(() => {
    ...
    if (guardados) {
      const cols = guardados.columnas || [];
      const colNums: string[] = [];
      for (const col of cols) {
        const sample = (guardados.datosLimpios || []).slice(0, 50).filter((r: any) => r[col] !== null && r[col] !== undefined && r[col] !== "" && r[col] !== "NaN" && r[col] !== "null" && r[col] !== "None" && r[col] !== "nan");
        if (sample.length > 0 && sample.every((r: any) => !isNaN(Number(String(r[col]).replace(",", "."))))) {
          colNums.push(col);
        }
      }
      const limpios = (guardados.datosLimpios || []).map((row: any) => {
        const fixed: any = { ...row };
        for (const col of cols) {
          const val = fixed[col];
          if (val === null || val === undefined || val === "" || val === "NaN" || val === "null" || val === "None" || val === "nan") {
            fixed[col] = colNums.includes(col) ? 0 : "Sin dato";
          }
        }
        return fixed;
      });
      ...
      setColumnas(cols);
      setColumnasOriginales(guardados.columnasOriginales || cols);
      setDatosOriginales(guardados.datosOriginales);
      setDatosLimpios(limpios);
      setNombreArchivo(guardados.nombreArchivo);
      setCsvRaw(guardados.csvRaw);
      setColumnasVisibles(new Set(cols));
  ...
  const ejecutarLimpieza = async (raw: string, nombre: string) => {
    ...
    try {
      ...
      const colNumericasDetectadas: string[] = [];
      if (parsed.columnas) {
        for (const col of parsed.columnas) {
          const sample = (parsed.limpios || []).slice(0, 50).filter((r: any) => r[col] !== null && r[col] !== undefined && r[col] !== "" && r[col] !== "NaN" && r[col] !== "null" && r[col] !== "None" && r[col] !== "nan");
          if (sample.length > 0 && sample.every((r: any) => !isNaN(Number(String(r[col]).replace(",", "."))))) {
            colNumericasDetectadas.push(col);
          }
        }
      }

      const datosLimpiosCorregidos = (parsed.limpios || []).map((row: any) => {
        const fixed: any = { ...row };
        for (const col of parsed.columnas) {
          const val = fixed[col];
          const esVacio = val === null || val === undefined || val === "" || val === "NaN" || val === "null" || val === "None" || val === "nan";
          if (esVacio) {
            fixed[col] = colNumericasDetectadas.includes(col) ? 0 : "Sin dato";
          }
        }
        return fixed;
      });
      ...
      setColumnasOriginales(parsed.columnas_originales || parsed.columnas);
      setColumnasVisibles(new Set(parsed.columnas));
      setDatosOriginales(parsed.originales);
      setDatosLimpios(datosLimpiosCorregidos);
      setPaginaActual(1);

      guardarDatos({
        columnas: parsed.columnas,
        columnasOriginales: parsed.columnas_originales || parsed.columnas,
        datosOriginales: parsed.originales,
        datosLimpios: datosLimpiosCorregidos,
        csvRaw: raw,
  ...
  const columnasVisiblesArr = useMemo(() => {
    if (vistaActiva === "originales") {
      return columnasOriginales.length > 0 ? columnasOriginales : columnas;
    }
    return columnas.filter((col) => columnasVisibles.has(col));
  }, [columnas, columnasOriginales, columnasVisibles, vistaActiva]);
      ...
      {!csvRaw ? (
        <div>
          <div
            ...
            onDrop={handleDrop}
          >
            <input id="pandas-file" name="pandas-file" type="file" ref={fileInputRef} onChange={handleFileChange} accept=".csv" style={{ display: "none" }} />
        ...
        <>
          ...
          <div className="btn-row table-controls" style={{ flexWrap: "wrap", gap: "0.75rem", padding: "12px 0", borderBottom: "1px solid #e2e8f0", marginBottom: "1rem" }}>
                ...
                <div style={{ position: "relative" }}>
                  <button
                    ...
                    style={{
                      ...
                      background: showColToggles ? "#6366f1" : "#fff",
                      color: showColToggles ? "#fff" : "#475569",
                      fontSize: "0.85rem",
                      cursor: "pointer",
                      fontWeight: 600,
            ...
            <div className="table-actions">
              {vistaActiva !== "reporte" && (
                <>
                  <Button onClick={() => abrirVentanaDatos("Visualizador de Datos", columnas, datosOriginales, datosLimpios, fullReport)} variant="secondary">Ver Completo</Button>
                </>
              )}
            </div>
          </div>
