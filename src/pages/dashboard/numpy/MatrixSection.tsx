      ...
      <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "12px" }}>
        {numericColumns.map((col) => (
          <label key={col} style={{ display: "flex", alignItems: "center", gap: "4px", padding: "6px 10px", borderRadius: "6px", border: "1px solid #e2e8f0", cursor: "pointer", fontSize: "13px", background: matrixColumns.includes(col) ? "#eef2ff" : "#fff" }}>
            <input id={`matrix-col-${col}`} name={`matrix-col-${col}`} type="checkbox" checked={matrixColumns.includes(col)} onChange={(e) => toggleColumn(col, e.target.checked)} />
            {col}
          </label>
        ))}
      </div>
      <div style={{ display: "flex", gap: "12px", alignItems: "center", flexWrap: "wrap" }}>
        <select id="matrix-operation" name="matrix-operation" value={matrixOperation} onChange={(e) => onMatrixOperationChange(e.target.value)} style={{ padding: "10px 14px", borderRadius: "6px", fontSize: "14px" }}>
