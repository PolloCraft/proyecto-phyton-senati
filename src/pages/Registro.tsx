...
function Registro() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (data.code) {
        navigate(`/Verificar?email=${encodeURIComponent(email)}&code=${data.code}`);
      } else {
  ...
  return (
    <AuthForm
      ...
      linkTo="/Login"
    >
      <FormField id="name" label="Nombre completo" placeholder="Tu nombre" value={name} onChange={setName} />
