...
def create_bar_chart(ax, data, labels, col, x_label, palette):
    """Create professional bar chart highlighting the most important values"""
    n = len(data)
    x = np.arange(n)
    ...
    for v in data:
        if v == max_val:
            colors.append('#dc2626')  # Red for highest
        elif v == min_val and min_val != max_val:
            colors.append('#059669')  # Green for lowest
        else:
            colors.append('#2563eb')  # Blue for others
    ...
    # Always annotate the max value prominently
    max_idx = np.argmax(data)
    ax.annotate(f'MAX: {data[max_idx]:.1f}',
                xy=(max_idx, data[max_idx]),
                xytext=(max_idx, data[max_idx] + max_val * 0.12),
                fontsize=10, fontweight='bold', color='#dc2626',
                ha='center', va='bottom',
                bbox=dict(boxstyle='round,pad=0.3', facecolor='#fef2f2', edgecolor='#dc2626', alpha=0.9))
    
    # Show value labels only on bars >= 70% of max
    if n <= 30:
        threshold = max_val * 0.7
        for i, bar in enumerate(bars):
            h = bar.get_height()
            if h >= threshold or i == max_idx:
                fontsize = 9 if i == max_idx else 8
                fw = 'bold' if i == max_idx else '500'
                color = '#dc2626' if i == max_idx else '#334155'
                ax.text(bar.get_x() + bar.get_width()/2., h + h*0.01, f'{h:.1f}',
                        ha='center', va='bottom', fontsize=fontsize, fontweight=fw, color=color)
    
    # Add statistics lines
    median_val = np.median(data)
    ax.axhline(mean_val, color='#dc2626', linestyle='--', linewidth=1.5, alpha=0.6, label=f'Media: {mean_val:.2f}')
    ax.axhline(median_val, color='#059669', linestyle=':', linewidth=1.5, alpha=0.6, label=f'Mediana: {median_val:.2f}')
    ax.legend(loc='upper right', fontsize=9)
    
    return {'mean': mean_val, 'median': median_val}

def create_line_chart(ax, data, labels, col, x_label, palette):
    """Create professional line chart with trend, highlighting important points"""
    n = len(data)
    x = np.arange(n)
    ...
    ax.plot(max_idx, data[max_idx], 'o', color='#dc2626', markersize=12, zorder=5, markerfacecolor='#fef2f2', markeredgewidth=2.5)
    ax.annotate(f'MAX: {data[max_idx]:.1f}',
                xy=(max_idx, data[max_idx]),
                xytext=(max_idx + max(1, n * 0.08), data[max_idx] + (np.max(data) - np.min(data)) * 0.1),
                fontsize=10, fontweight='bold', color='#dc2626',
                arrowprops=dict(arrowstyle='->', color='#dc2626', lw=1.5),
                bbox=dict(boxstyle='round,pad=0.3', facecolor='#fef2f2', edgecolor='#dc2626', alpha=0.9))
    
    if min_idx != max_idx:
        ax.plot(min_idx, data[min_idx], 'o', color='#059669', markersize=10, zorder=5, markerfacecolor='#f0fdf4', markeredgewidth=2.5)
        ax.annotate(f'MIN: {data[min_idx]:.1f}',
                    xy=(min_idx, data[min_idx]),
                    xytext=(min_idx + max(1, n * 0.08), data[min_idx] - (np.max(data) - np.min(data)) * 0.1),
                    fontsize=9, fontweight='bold', color='#059669',
                    arrowprops=dict(arrowstyle='->', color='#059669', lw=1.5),
                    bbox=dict(boxstyle='round,pad=0.3', facecolor='#f0fdf4', edgecolor='#059669', alpha=0.9))
...
def create_scatter_chart(ax, data, labels, col, x_label, palette):
    """Create professional scatter plot highlighting outliers and key points"""
    n = len(data)
    x = np.arange(n)
    ...
    if normal_mask.any():
        ax.scatter(x[normal_mask], data[normal_mask], c=data[normal_mask], cmap='viridis', s=60,
                   edgecolors='white', linewidth=0.8, alpha=0.8, label='Normal', zorder=3)
    if outlier_mask.any():
        ax.scatter(x[outlier_mask], data[outlier_mask], c='#dc2626', s=100, marker='D',
                   edgecolors='white', linewidth=1.5, alpha=0.9, label='Outlier', zorder=4)
    
    # Highlight max and min
    max_idx = np.argmax(data)
    min_idx = np.argmin(data)
    ax.annotate(f'MAX: {data[max_idx]:.1f}', xy=(max_idx, data[max_idx]),
                xytext=(max_idx + max(1, n * 0.05), data[max_idx] + (np.max(data) - np.min(data)) * 0.08),
                fontsize=9, fontweight='bold', color='#dc2626',
                arrowprops=dict(arrowstyle='->', color='#dc2626', lw=1.2),
                bbox=dict(boxstyle='round,pad=0.2', facecolor='#fef2f2', edgecolor='#dc2626', alpha=0.85))
    
    # Trend line
    if n >= 2:
        z = np.polyfit(x, data, 1)
        p = np.poly1d(z)
        ax.plot(x, p(x), color='#dc2626', linestyle='--', linewidth=2, alpha=0.8, label='Tendencia lineal')
    ...
    sc = ax.scatter(x, data, c=data, cmap='viridis', s=0, alpha=0)
...
def create_pie_chart(ax, data, labels, col, palette):
    """Create professional pie/donut chart highlighting the top category"""
    ...
    # Explode the top slice slightly
    ...
    for i, t in enumerate(texts):
        t.set_fontsize(9 if i > 0 else 10)
        t.set_color('#334155')
        t.set_fontweight('bold' if i == 0 else 'normal')
    for i, at in enumerate(autotexts):
        at.set_fontsize(8 if i > 0 else 9)
        at.set_color('white')
        at.set_fontweight('bold')
    
    # Annotate top category
    top_count = counts.values[0]
    top_pct = top_count / counts.sum() * 100
    ax.text(0, -1.35, f'Mayor: {counts.index[0]} ({top_pct:.1f}%)',
            ha='center', fontsize=10, fontweight='bold', color='#1e3a5f',
            bbox=dict(boxstyle='round,pad=0.3', facecolor='#eef2ff', edgecolor='#2563eb', alpha=0.9))
