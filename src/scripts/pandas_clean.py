...
if incorrect_rows:
    bad_indices = [r["row_index"] for r in incorrect_rows]
...
for col in df.columns:
    ...
    if col_info['type'] == 'numeric' or col in numeric_cols_inferred:
        ...
        if action == 'fill_mean':
            ...
            fill_val = 0.0
        df[col] = s_num.fillna(fill_val)
    else:
        s_str = df[col].astype(str).str.strip()
        s_str = s_str.replace({'nan': '', 'None': '', 'NoneType': ''})
        if is_date_col:
            for idx, val in s_str.items():
                if '/' in val and re.match(r'^\d{4}/\d{2}/\d{2}$', val):
                    s_str.at[idx] = val.replace('/', '-')
        # Normalize case for category columns (short unique value lists)
        unique_vals = s_str[s_str != ''].unique()
        if len(unique_vals) > 0 and len(unique_vals) <= 20 and not is_date_col:
            ...
            if len(set(lower_unique)) < len(unique_vals):
                s_str = s_str.apply(lambda x: x.title() if x != '' else x)
        if action == 'fill_mode':
            mode_val = s_str[s_str != ''].mode()
            fill_val = mode_val.iloc[0] if len(mode_val) > 0 else ''
            df[col] = s_str.replace('', fill_val)
        else:
            df[col] = s_str
...
# Keep only numeric columns for cleaned data (for NumPy processing)
df_limpio = df[numeric_cols_inferred].copy() if numeric_cols_inferred else df.copy()
