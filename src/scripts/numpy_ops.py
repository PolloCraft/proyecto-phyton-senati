...
vec_a = np.array([float(str(r.get(col_a, 0)).replace(',', '.')) for r in data], dtype=float)
_output = ""
...
if op == "create":
    ...
    matrix_data = {}
    for c in cols:
        matrix_data[c] = [float(str(r.get(c, 0)).replace(',', '.')) for r in data]
    ...
    vec_b = np.array([float(str(r.get(col_b, 0)).replace(',', '.')) for r in data], dtype=float)
